...
const ChartTooltip = ({ active, payload, mode }) => {
  ...
  const wrapperCls = "bg-card border border-border rounded-2xl p-3 shadow-none";
  const lbl = <p className="text-muted text-[10px] font-bold mb-1 uppercase tracking-wider">{formatDate(data.fecha)}</p>;

  if (mode === '1rm') return (
    <div className={wrapperCls}>{lbl}
      <p className="text-signal-orange font-condensed font-black text-2xl leading-none mb-1">
        {data.valor}<span className="text-sm font-sans font-normal text-muted ml-0.5">kg</span>
      </p>
      <p className="text-ink text-xs font-bold">{data.cargaReal}kg × {data.repsReales}</p>
    </div>
  );
  if (mode === 'vol') return (
    <div className={wrapperCls}>{lbl}
      <p className="text-signal-orange font-condensed font-black text-2xl leading-none">
        {data.volumenTotal?.toLocaleString()}<span className="text-sm font-sans font-normal text-muted ml-0.5">kg</span>
      </p>
      <p className="text-[10px] text-muted font-bold mt-1">Volumen Total</p>
    </div>
  );
  if (mode === 'rpe') return (
    <div className={wrapperCls}>{lbl}
      <p className="text-ink font-condensed font-black text-2xl leading-none">
        {data.rpeMedio}<span className="text-sm font-sans font-normal text-muted ml-0.5">/ 10</span>
      </p>
      <p className="text-[10px] text-muted font-bold mt-1">Fatiga RPE Medio</p>
    </div>
  );
  if (mode === 'meso') return (
    <div className={`${wrapperCls} border-l-4`} style={{ borderLeftColor: data.mesoColor }}>{lbl}
      <p className="text-ink font-condensed font-black text-2xl leading-none">
        {data.maxPR}<span className="text-sm font-sans font-normal text-muted ml-0.5">kg</span>
      </p>
      <p className="text-[10px] text-muted font-bold mt-1">PR Máximo</p>
    </div>
  );
  return null;
};
...
const Sparkline = ({ data }) => {
  ...
  return (
    <svg width={W} height={H} className="overflow-visible">
      ...
      <polyline points={polyline} fill="none" stroke="var(--color-signal-orange)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx={pts[pts.length-1].x} cy={pts[pts.length-1].y} r="3" fill="var(--color-signal-orange)" stroke="white" strokeWidth="1.5" />
    </svg>
  );
};
...
const HistorySheet = ({ exerciseName, history, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="bg-card w-full rounded-t-[32px] pt-4 pb-12 px-5 flex flex-col shadow-2xl relative z-10 animate-slide-up h-[80vh] border-t border-border">
        <div className="w-10 h-1 bg-[#E8E8E4] rounded-full mx-auto mb-5" />
        <div className="flex justify-between items-center mb-5">
          <div>
            <p className="text-[10px] font-condensed font-bold text-muted tracking-widest uppercase mb-0.5">Historial</p>
            <h3 className="font-condensed font-black text-2xl text-ink">{exerciseName}</h3>
          </div>
          <button onClick={onClose} className="p-2 bg-bg rounded-full text-muted hover:text-ink transition-colors">
            <X size={20} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto hide-scrollbar -mx-1 px-1 space-y-2.5">
          {history.length === 0 ? (
            <p className="text-center text-muted mt-10">No hay registros</p>
          ) : history.map(record => {
            const isPR = record.valor === maxValor;
            return (
              <div key={record.id} className={`bg-bg rounded-2xl border flex justify-between items-center px-4 py-3.5 ${isPR ? 'border-[#FF6B00] border-l-4' : 'border-border'}`}>
                <div>
                  <div className="text-muted text-xs font-bold mb-1">{formatDate(record.fecha)}</div>
                  <div className="text-ink font-condensed font-bold text-xl">
                    {record.cargaReal}kg × {record.repsReales}
                    <span className="text-muted text-sm ml-1 font-sans font-normal">reps</span>
                  </div>
                </div>
                <div className="text-right">
                  {isPR && <span className="inline-block bg-[#FF6B00] text-white font-black text-[10px] px-2 py-0.5 rounded-full mb-1">🏆 PR</span>}
                  <div className="font-condensed font-black text-2xl text-signal-orange">
                    {Math.round(record.valor)}<span className="text-sm font-sans font-normal text-muted ml-0.5">kg</span>
                  </div>
                  <div className="text-[10px] text-muted">1RM estimado</div>
                </div>
              </div>
            );
          })}
...
const SectionHeader = ({ title, sub }) => (
  <div className="mb-3">
    <h2 className="font-condensed font-black text-xl text-ink uppercase tracking-wide leading-none">{title}</h2>
...
const SessionDetailSheet = ({ log, onClose, handleRepeat }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      <div className="absolute inset-0 bg-black/40 animate-fade-in" onClick={onClose} />
      <div className="bg-card w-full rounded-t-[32px] pt-4 pb-8 px-5 flex flex-col shadow-2xl relative z-10 animate-slide-up h-[95vh] border-t border-border">
        <div className="w-10 h-1 bg-[#E8E8E4] rounded-full mx-auto mb-5" />
        ...
        <div className="flex justify-between items-start mb-5">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-[10px] font-bold text-muted uppercase tracking-wider">{formatDate(log.fecha)}</span>
            </div>
            <h3 className="font-condensed font-black text-3xl leading-none text-ink">{log.sessionName}</h3>
          </div>
          <button onClick={onClose} className="p-2 bg-bg rounded-full text-muted hover:text-ink transition-colors shrink-0">
            <X size={20} />
          </button>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-bg border border-border rounded-2xl p-3">
            <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-1">Volumen</p>
            <p className="font-condensed font-black text-2xl text-ink">{log.volumenTotal?.toLocaleString()}<span className="text-sm font-sans font-normal ml-0.5">kg</span></p>
          </div>
          <div className="bg-bg border border-border rounded-2xl p-3">
            <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-1">RPE Medio</p>
            <p className="font-condensed font-black text-2xl text-ink">{log.rpeMedio}<span className="text-sm font-sans font-normal ml-0.5">/10</span></p>
          </div>
          <div className="bg-bg border border-border rounded-2xl p-3">
            <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-1">Duración</p>
            <p className="font-condensed font-black text-2xl text-ink">{Math.floor((log.duracion || 0)/60)}<span className="text-sm font-sans font-normal ml-0.5">min</span></p>
          </div>
          <div className="bg-bg border border-border rounded-2xl p-3">
            <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-1">Ejercicios</p>
            <p className="font-condensed font-black text-2xl text-ink">{log.ejerciciosCompletados}<span className="text-sm font-sans font-normal text-muted ml-1">/ {log.ejerciciosTotal}</span></p>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto hide-scrollbar -mx-2 px-2 pb-4">
          <h4 className="font-condensed font-bold text-lg text-ink uppercase tracking-wide mb-3">Desglose</h4>
          {(log.ejercicios || []).map((ex, i) => (
            <div key={i} className="mb-4">
              <p className="font-bold text-ink text-sm mb-2">{ex.nombre}</p>
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                {(ex.seriesLog || []).filter(s => s.done).map((serie, sIdx) => (
                  <div key={sIdx} className="flex items-center px-4 py-2 border-b border-border last:border-b-0 text-sm">
                    <div className="w-10 text-muted font-condensed font-bold">S{sIdx + 1}</div>
                    <div className="w-20 font-bold text-ink">{serie.carga} kg</div>
                    <div className="w-20 font-bold text-ink">{serie.reps} reps</div>
                ...
                {(!ex.seriesLog || ex.seriesLog.filter(s => s.done).length === 0) && (
                  <div className="px-4 py-3 text-sm text-muted italic">No se registró carga</div>
                )}
              </div>
            </div>
          ))}
          ...
          <div className="mt-4 border-t border-border pt-4">
            <h4 className="font-condensed font-bold text-lg text-ink uppercase tracking-wide mb-3">
        ...
        <div className="pt-4 flex gap-3">
          <button onClick={handleShareLocal} className="w-14 shrink-0 bg-card border border-border text-muted rounded-2xl flex items-center justify-center hover:border-[#6E6E73] active:scale-95 transition-all">
            <Share2 size={20} />
          </button>
          <button onClick={() => handleRepeat(log)} className="flex-1 bg-[#FF6B00] text-white font-condensed font-black text-xl rounded-2xl py-3.5 shadow-none active:scale-[0.98] transition-transform flex justify-center items-center gap-2">
...
export default function Evolution() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg">
      ...
      {/* HEADER */}
      <div className="px-5 pt-6 pb-3 sticky top-0 bg-card border-b border-border z-30">
        <div className="flex justify-between items-center mb-4">
          <div>
            <p className="text-[10px] font-condensed font-bold text-signal-orange tracking-widest uppercase mb-0.5">TrainingOS</p>
        ...
        {/* Tabs */}
        <div className="flex gap-1 bg-bg p-1 rounded-2xl">
      ...
      <div className="flex-1 px-4 pb-24 overflow-y-auto pt-4">
        ...
        {activeTab === 'historial' && (
          <div className="animate-fade-in-up">
            {/* STATS GLOBALES */}
            <div className="bg-card border border-border rounded-2xl flex divide-x divide-border shadow-none mb-4">
              <div className="flex-1 py-3 px-2 text-center">
                <div className="text-[10px] text-muted font-condensed font-bold uppercase tracking-widest mb-0.5">TOTAL</div>
                <div className="font-condensed font-black text-ink text-xl leading-none">{tTotalSes} <span className="text-xs font-sans font-normal text-muted">ses.</span></div>
              </div>
              <div className="flex-1 py-3 px-2 text-center">
                <div className="text-[10px] text-muted font-condensed font-bold uppercase tracking-widest mb-0.5">VOL MED</div>
                <div className="font-condensed font-black text-ink text-xl leading-none">{Math.round(tVolMed).toLocaleString()} <span className="text-xs font-sans font-normal text-muted">kg</span></div>
              </div>
              <div className="flex-1 py-3 px-2 text-center">
                <div className="text-[10px] text-muted font-condensed font-bold uppercase tracking-widest mb-0.5">RACHA</div>
                <div className="font-condensed font-black text-signal-orange text-xl leading-none">{activeStreak > 2 ? '🔥' : ''} {activeStreak} <span className="text-xs font-sans font-normal text-muted">días</span></div>
              </div>
            </div>
            ...
            <div className="relative mb-5">
              ...
              <input 
                ...
                className="w-full bg-card border border-border rounded-xl py-3 pl-10 pr-4 text-sm font-bold placeholder:font-normal focus:border-[#FF6B00] outline-none shadow-none"
              />
            </div>
            ...
            {sessionLogs.length === 0 ? (
              <div className="mt-8 flex flex-col items-center justify-center text-center px-6">
                <div className="w-24 h-24 bg-signal-orange/10 border border-border rounded-3xl flex items-center justify-center mb-5">
                  <span className="text-5xl">📋</span>
                </div>
                <h3 className="font-condensed font-black text-2xl text-ink mb-2">Sin sesiones registradas</h3>
              ...
              <div className="text-center text-muted mt-10 text-sm">No se encontraron sesiones con esos filtros.</div>
            ) : (
              historyGroups.map((group, gIdx) => (
                <div key={gIdx} className="mb-6">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-[10px] font-condensed font-bold uppercase tracking-widest text-muted">{group.label}</span>
                    <div className="flex-1 h-px bg-[#E8E8E4]" />
                  </div>
                    ...
                    {group.sessions.map((log, lIdx) => {
                      ...
                      return (
                        <div 
                          ...
                          className={`bg-card border border-border rounded-2xl overflow-hidden shadow-none active:scale-[0.98] transition-transform stagger-${Math.min(lIdx+1, 7)} relative`}
                        >
                          <div className="bg-bg px-4 py-2 border-b border-border flex justify-between items-center">
                          ...
                          <div className="p-4">
                            <div className="flex justify-between items-start mb-4">
                              <h3 className="font-condensed font-black text-xl text-ink leading-tight pr-4">{log.sessionName}</h3>
                              <div className="text-right shrink-0 bg-bg px-2 py-1 rounded-lg">
                                <div className="text-ink font-condensed font-bold text-lg leading-none">{Math.floor((log.duracion || 0)/60)}<span className="text-xs font-sans ml-0.5">m</span></div>
                              </div>
                            </div>
                            
                            <div className="flex justify-between items-center border-t border-border pt-3">
                              <div className="flex flex-col">
                                <span className="text-[10px] text-muted font-bold uppercase">Volumen</span>
                                <span className="text-ink font-condensed font-black text-lg leading-none">📦 {(log.volumenTotal || 0).toLocaleString()} <span className="text-xs font-sans text-muted font-normal">kg</span></span>
                              </div>
                              <div className="flex flex-col">
                                <span className="text-[10px] text-muted font-bold uppercase">RPE</span>
                                <span className="font-condensed font-black text-lg leading-none" style={{ color: rpeC }}>🔋 {log.rpeMedio}</span>
                              </div>
                              <div className="flex flex-col pr-1">
                                <span className="text-[10px] text-muted font-bold uppercase">Ejercicios</span>
                                <span className="text-ink font-condensed font-black text-lg leading-none">✅ {log.ejerciciosCompletados}/{log.ejerciciosTotal}</span>
                              </div>
                            </div>
                          </div>
                        </div>
        ...
        {activeTab === 'prs' && filteredCards.length === 0 && (
          <div className="mt-8 flex flex-col items-center justify-center animate-fade-in-up text-center px-6">
            ...
            <h3 className="font-condensed font-black text-2xl text-ink mb-2">
        ...
        {activeTab === 'prs' && filteredCards.length > 0 && (
          <div className="space-y-3 animate-fade-in-up">
            {filteredCards.map((card, ci) => (
              <div
                key={card.id}
                className={`bg-card border border-border rounded-2xl overflow-hidden shadow-none relative flex stagger-${Math.min(ci+1,7)}`}
              >
                {/* Green left bar */}
                ...
                <div className="p-4 flex-1">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h3 className="font-sans font-bold text-base text-ink leading-tight">{card.name}</h3>
                      <div className="flex gap-2 mt-1.5">
                        <span className="text-[10px] font-black tracking-wider border border-border text-muted px-2 py-0.5 rounded-lg uppercase">
                          {card.cat}
                        </span>
                        <span className="text-[10px] font-black tracking-wider border border-border text-muted px-2 py-0.5 rounded-lg uppercase">
                          {card.sport}
                    ...
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center bg-bg border border-border rounded-full text-muted hover:text-ink hover:border-[#6E6E73] transition-colors"
                    >
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      <div className="text-[10px] font-condensed font-bold text-muted tracking-widest uppercase mb-1">
                        1RM ESTIMADO
                      </div>
                      <div className="font-display font-black text-5xl leading-none text-signal-orange uppercase tracking-wide tabular-nums">
                        {card.valor}
                        <span className="text-lg font-sans font-normal text-muted ml-1">kg</span>
                      </div>
                      <div className="text-sm text-muted font-medium mt-1">
                        {card.carga}kg × {card.reps} reps
                        <span className="text-xs text-muted/60 ml-2">· {relativeDate(card.fecha)}</span>
                      </div>
                    </div>
        ...
        {activeTab === 'graficas' && !hasData && (
          <div className="animate-fade-in-up space-y-4">
            ...
            <div className="mt-4 flex flex-col items-center justify-center text-center px-6">
              ...
              <h3 className="font-condensed font-black text-2xl text-ink mb-2">Tus gráficas aparecerán aquí</h3>
              <p className="text-muted text-sm max-w-[280px] leading-relaxed">
              ...
              <div style={{ width: '100%', height: 200 }}>
                <ResponsiveContainer>
                  ...
                  >
                    ...
                    <Line type="monotone" dataKey="valor" stroke="var(--color-signal-orange)" strokeWidth={2.5} strokeDasharray="6 3" dot={{ r: 4, fill: '#FF6B00', stroke: 'white', strokeWidth: 2 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-5 shadow-none relative">
              ...
              <div style={{ width: '100%', height: 180 }}>
                <ResponsiveContainer>
                  ...
                  >
                    <XAxis dataKey="fecha" tickFormatter={formatShortDate} tick={{ fill: '#6E6E73', fontSize: 10 }} stroke="var(--color-border)" tickMargin={8} />
                    <Bar dataKey="volumenTotal" fill="var(--color-signal-orange)" radius={[6,6,0,0]} opacity={0.5} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
                  ...
                  <div style={{ width: '100%', height: 180 }}>
                    <ResponsiveContainer>
                      <BarChart data={sessionLogs} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                        ...
                        <Bar dataKey="volumenTotal" fill="var(--color-signal-orange)" radius={[6, 6, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                ),
                  ...
                  <div style={{ width: '100%', height: 180 }}>
                    <ResponsiveContainer>
                      <LineChart data={sessionLogs} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" vertical={false} />
                        <ReferenceArea y1={7} y2={8.5} fill="var(--color-signal-orange)" fillOpacity={0.06} />
                        <XAxis dataKey="fecha" tickFormatter={formatShortDate} tick={{ fill: '#6E6E73', fontSize: 10 }} stroke="var(--color-border)" tickMargin={8} />
                        <YAxis domain={[5, 10]} ticks={[5,6,7,8,9,10]} tick={{ fill: '#6E6E73', fontSize: 10 }} stroke="var(--color-border)" />
          ...
          <div className="space-y-4 animate-fade-in-up">
            {activeTestSubTab === 'wellness' && (
              <div className="bg-card border border-border rounded-2xl p-5 shadow-none space-y-4">
                ...
                {wellnessLogs.length > 0 ? (
                    ...
                    <div style={{ width: '100%', height: 180 }}>
                      <ResponsiveContainer>
                        <LineChart data={[...wellnessLogs].reverse()} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" vertical={false} />
                          <XAxis dataKey="fecha" tickFormatter={formatShortDate} tick={{ fill: '#6E6E73', fontSize: 10 }} stroke="var(--color-border)" />
                          <YAxis domain={[1, 5]} ticks={[1,2,3,4,5]} tick={{ fill: '#6E6E73', fontSize: 10 }} stroke="var(--color-border)" />
                          <Tooltip content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                              const data = payload[0].payload;
                              return (
                                <div className="bg-card border border-border p-3 rounded-xl shadow-lg text-xs space-y-1 font-sans">
                                  <p className="font-bold text-ink">{formatDate(data.fecha)}</p>
                                  <p className="text-corner-blue font-semibold">Sueño: {data.sleep}/5</p>
                                  <p className="text-corner-blue font-semibold">Estrés: {data.stress}/5</p>
                                  <p className="text-red-500 font-semibold">Agujetas: {data.doms}/5</p>
                                  <p className="text-signal-orange font-semibold">Fatiga: {data.fatigue}/5</p>
                                </div>
                              );
                            }
                            return null;
                          }} />
                          <Line type="monotone" name="Sueño" dataKey="sleep" stroke="var(--color-corner-blue)" strokeWidth={2} dot={{ r: 3 }} />
                          <Line type="monotone" name="Fatiga" dataKey="fatigue" stroke="var(--color-signal-orange)" strokeWidth={2} dot={{ r: 3 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                        ...
                        return (
                          <div key={log.id} className="flex justify-between items-center py-2 border-b border-border text-xs">
                            <div>
                              ...
                              <span className="text-muted">Sueño: {log.sleep} · Estrés: {log.stress} · Agujetas: {log.doms} · Fatiga: {log.fatigue}</span>
                            </div>
                            <span className={`font-condensed font-black text-sm px-2 py-0.5 rounded ${
                              score >= 4 ? 'bg-success-green/10 text-success-green' : score >= 2.5 ? 'bg-signal-orange/10 text-signal-orange' : 'bg-corner-red/10 text-corner-red'
                            }`}>
                              {score.toFixed(1)}/5
                            </span>
                          </div>
                  ...
                  <div className="h-32 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl text-muted text-xs p-4 text-center">
                    <span>Sin registros de check-in Wellness.</span>
                    <span className="text-[10px] text-muted mt-1">Completa tu check-in diario al iniciar una sesión.</span>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTestSubTab === 'cmj' && (
              <div className="bg-card border border-border rounded-2xl p-5 shadow-none space-y-4">
                ...
                {cmjLogs.length > 0 ? (
                  <>
                    <div className="bg-bg/25 border border-border rounded-xl p-4 flex justify-between items-center">
                      <div>
                        <span className="text-[10px] text-muted font-bold block uppercase tracking-wider">Media 30 Días</span>
                        <span className="font-condensed font-black text-2xl text-ink">{cmjStats.avg30d} cm</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] text-muted font-bold block uppercase tracking-wider">Último Salto</span>
                        <span className="font-condensed font-black text-2xl text-signal-orange">{cmjStats.lastJump} cm</span>
                      </div>
                    </div>
                        ...
                        <LineChart data={[...cmjLogs].reverse()} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                          ...
                          <Tooltip content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                              const data = payload[0].payload;
                              return (
                                <div className="bg-card border border-border p-3 rounded-xl shadow-lg text-xs font-sans">
                                  <p className="font-bold text-ink">{formatDate(data.fecha)}</p>
                                  <p className="text-signal-orange font-black">Salto: {data.valor} cm</p>
                                </div>
                              );
                            }
                            return null;
                          }} />
                          <Line type="monotone" dataKey="valor" stroke="var(--color-signal-orange)" strokeWidth={2.5} dot={{ r: 4, fill: '#FF6B00', stroke: 'white', strokeWidth: 2 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  ...
                  <div className="h-32 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl text-muted text-xs p-4 text-center">
                    ...
                    <span className="text-[10px] text-muted mt-1">Registra tu test de salto en el check-in diario.</span>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="bg-card border border-border rounded-2xl p-5 shadow-none space-y-4">
                ...
                {cardioTests.length > 0 ? (
                          ...
                          <Tooltip content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                              ...
                              return (
                                <div className="bg-card border border-border p-3 rounded-xl shadow-lg text-xs space-y-1 font-sans">
                                  <p className="font-bold text-ink">{formatDate(data.fecha)}</p>
                                  <p className="text-success-green font-bold">VO2Max: {data.valor} ml/kg/min</p>
                                  <p className="text-muted font-semibold">Test: {data.tipo === 'cooper' ? `Cooper (${data.valorOriginal}m)` : `Beep Test (${data.valorOriginal} km/h)`}</p>
                                </div>
                              );
                            }
                            return null;
                          }} />
                          <Line type="monotone" dataKey="valor" stroke="var(--color-success-green)" strokeWidth={2.5} dot={{ r: 4, fill: '#34C759', stroke: 'white', strokeWidth: 2 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  ...
                  <div className="h-32 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl text-muted text-xs p-4 text-center">
                    ...
                    <span className="text-[10px] text-muted mt-1">Pídele a tu Coach que te planifique un Test de Cooper o Beep Test.</span>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTestSubTab === 'composition' && (
              <div className="bg-card border border-border rounded-2xl p-5 shadow-none space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  {!showAddMetrics && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-signal-orange/10 text-signal-orange border border-border rounded-xl font-condensed font-black text-xs hover:bg-[#FF6B00] hover:text-white transition-colors"
                    >
                      + NUEVO REGISTRO
                    </button>
                  )}
                ...
                {showAddMetrics && (
                  <form onSubmit={handleSaveMetrics} className="bg-bg border border-border rounded-2xl p-4 space-y-3 animate-fade-in-up">
                    <h4 className="font-condensed font-black text-xs text-ink uppercase tracking-wider">
                      Registrar Composición Corporal
                    </h4>
                    
                    <div className="mb-3">
                      <label className="block text-[10px] text-muted font-bold uppercase mb-1">Fecha de Registro</label>
                      <input
                        type="date"
                        required
                        value={metricDate}
                        onChange={e => setMetricDate(e.target.value)}
                        className="w-full bg-card border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#FF6B00]"
                      />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] text-muted font-bold uppercase mb-1">Peso (kg) *</label>
                        <input
                          type="number"
                          step="0.1"
                          required
                      ...
                      <div>
                        <label className="block text-[10px] text-muted font-bold uppercase mb-1">Grasa (%)</label>
                        <input
                          type="number"
                          step="0.1"
                          value={metricFat}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <label className="block text-[9px] text-muted font-bold uppercase mb-1">Cintura (cm)</label>
                        <input
                          type="number"
                          step="0.5"
                          value={metricWaist}
                          ...
                          placeholder="82"
                          className="w-full bg-card border border-border rounded-xl px-2.5 py-1.5 text-xs focus:outline-none focus:border-[#FF6B00]"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] text-muted font-bold uppercase mb-1">Brazo (cm)</label>
                        <input
                          type="number"
                          step="0.5"
                          value={metricArm}
                      ...
                      <div>
                        <label className="block text-[9px] text-muted font-bold uppercase mb-1">Muslo (cm)</label>
                        <input
                          type="number"
                          step="0.5"
                          value={metricThigh}
                    ...
                    <div className="flex justify-end gap-2 pt-1">
                      <button
                        ...
                        className="px-3 py-1.5 border border-border rounded-xl font-condensed font-bold text-xs text-muted bg-card hover:bg-[#FAFAFA]"
                      >
                        CANCELAR
                      </button>
                      <button
                ...
                {bodyMetrics.length > 0 ? (
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {bodyMetrics.map(log => (
                      <div key={log.id} className="flex justify-between items-start py-2.5 border-b border-border text-xs">
                        <div>
                          <span className="font-bold text-ink block">{formatDate(log.fecha)}</span>
                        ...
                        <div className="text-right shrink-0">
                          <span className="font-condensed font-black text-sm text-ink block">
                            {log.peso} kg
                          </span>
                          {log.grasa && (
                            <span className="text-[10px] text-success-green font-bold block">
                              {log.grasa}% grasa
                            </span>
                          )}
                        </div>
                  ...
                  <div className="h-32 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl text-muted text-xs p-4 text-center">
                    ...
                    <span className="text-[10px] text-muted mt-1">Registra tu peso para ver tu progreso histórico aquí.</span>
                  </div>
                )}
              </div>
            )}
