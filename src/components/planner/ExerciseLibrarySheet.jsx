...
export default function ExerciseLibrarySheet({ open, onClose, onSelectExercise }) {
  ...
  return (
    <>
      ...
      {/* Sheet (85vh) */}
      <div
        className={`fixed bottom-0 left-0 w-full bg-white border-t border-border shadow-2xl rounded-t-3xl z-[80] transition-transform duration-300 ease-out flex flex-col`}
        style={{
          height: '85dvh',
      ...
      >
        ...
        <div className="px-5 pt-2 pb-3 border-b border-border/50 shrink-0">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-condensed font-black text-2xl text-text">Biblioteca de Ejercicios</h3>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-bg border border-border rounded-xl pl-10 pr-4 py-3 text-sm font-bold text-text placeholder:text-muted/50 outline-none focus:border-accent transition-colors"
            />
          </div>
        </div>

        {/* Categories (Scrollable) */}
        <div className="px-4 py-3 shrink-0 overflow-x-auto hide-scrollbar flex items-center gap-2 border-b border-border/50">
          {CATEGORIES.map(cat => (
            <button
              key={cat}
            ...
            <div className="flex flex-col gap-2">
              {filteredExercises.map(ex => (
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3.5 bg-card rounded-2xl border border-border hover:border-accent active:scale-[0.98] transition-all text-left shadow-sm"
                >
                  <div>
        ...
        {/* Sticky Bottom Create Button Footer */}
        <div className="shrink-0 p-4 bg-card border-t border-border">
          <button
            onClick={handleCreateEmpty}
            className="w-full py-3.5 bg-card border-2 border-dashed border-accent rounded-xl font-bold text-accent hover:bg-accent/5 shadow-sm active:scale-[0.98] transition-all flex items-center justify-center gap-2"
          >
